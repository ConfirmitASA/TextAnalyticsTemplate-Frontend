...
class Hitlist {
  ...
  addInfoText() {
    ...
    let infoTextClass = 'ta-info-text';
    let infoText = this.source.querySelector('.' + infoTextClass);
    ...
    let infoIcon = this.source.querySelector('.' + infoIconClass);
    ...
    if (!infoIcon) {
      ...
      infoIcon.onmouseover = () => {
        ...
        infoText.style.top = (rect.top + 32 + window.pageYOffset) + 'px';
        infoText.style.right = (window.innerWidth - rect.right - 32) + 'px';
        infoText.style.width = (container.clientWidth * 0.8) + 'px';
      };
